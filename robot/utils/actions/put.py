...
def put(lib: CDLL, device: dict, default_params: dict):
    """Функция положить."""
    err_text_c = c_char_p()
    # выполняем мерцание светодиодом, передаем дескриптор светодиода,
    # 3 параметра цвета(RGB), продолжительность,
    # кол-во повторений и включаем асинхронный режим работы
    if (
        err_code := lib.RI_SDK_exec_RGB_LED_Flicker(
            device["led"],
            0,
            0,
            255,
    ...
    if (
        err_code := lib.RI_SDK_exec_ServoDrive_Turn(
            device["claw"],
            default_params.get("claw_unclenched_position"),
            default_params.get("speed"),
            c_bool(False),
            err_text_c,
        )
